...
def _run_loop() -> None:
    ...
    while _stop_event is not None and not _stop_event.is_set():
        db = SessionLocal()
        try:
            service = CalendarSyncService(db=db, settings=settings)
            service.sync_all_users_for_today()
        except Exception:
            pass
        finally:
            db.close()
...
def start_calendar_sync_worker() -> None:
        ...
        try:
            ...
            if engine.dialect.name == 'postgresql':
                row = _leader_connection.execute(
                    ...
                    {'lock_id': settings.background_sync_leader_lock_id},
                ).first()
    ...
    _worker_thread = threading.Thread(target=_run_loop, name='calendar-sync-worker', daemon=True)
